...
class MazeEnvironment(Environment):
    def __init__(self, maze, init_position, goal_position):
        x = len(maze)
        y = len(maze)

        self.boundary = np.asarray([x,y])
        self.init_position = init_position
        self.current_position = np.asarray(self.init_position)
        ...
        self.distances = np.sqrt(np.sum((np.array(self.allowed_states) -
                                         np.asarray(self.goal_position))**2,
                                         axis = 1))
    ...
    def state_update(self, action):
        ...
        if (self.current_position == self.goal_position).all():
                reward = 1
                isgameon = False
    ...
    def draw(self):
        ...
        ax.plot(self.goal_position[1], self.goal_position[0],
                'bs', markersize = 4)
